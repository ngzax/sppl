...
class MatchesControllerTest < ActionController::TestCase
  ...
  context "When showing ALL the Matches in HTML" do
    ...
    should "provide a link to add a new Match" do
      assert_select "a", :content => "Add a New Match", :attributes => {:href => new_match_path}
    end
    ...
    should "be able to pass in a season_id as a filter" do
      get :index, :season_id => create(:season2).id
      assert_response :success
  ...
  context "When showing a single Match" do
    ...
    should "Correctly displays the Match's Id" do
      assert_select "table tr th", "Match Id:"
      assert_select "table tr td", "#{@match.id}"
    end
  ...
  context "When showing the form to create a New Match" do
    ...
    should "Correctly display the Header" do
      assert_select "h2", "Creating a Match"
    end

    should "Show that we can enter Ordinal, Start Year, and Stop Year" do
      assert_select "label", "Match date"
      assert_select "input[id=match_match_date]"
    end

    should "show a dropdown to associate the Match with a Season" do
      assert_select "label", "Season"
      assert_select "select[id='match_season_id']"
    end
  end
