...
class PlayersControllerTest < ActionController::TestCase
  ...
  setup do
    ...
    @m = create(:match, :season => @season)
    @g = create(:game, :match => @m)
  ...
  context "When showing ALL the Players in HTML" do
    ...
    should "link the Player's full name to the Show action" do
      assert_select "a", :content => "#{@player.full_name}", :attributes => {:href => player_path(@player.id)}
    end
    ...
    should "NOT show the Actions column and links" do
      assert_select "a", :content => "Change", :attributes => {:href => edit_player_path(@player.id)}
      assert_select "a", :content => "Remove"
    end

    should "NOT show the link to add a new Player" do
      assert_select "a", {:content => "Add a New Player", :attributes => {:href => new_player_path}}, false
    end
  ...
  context "AND logged in as Admin" do
    ...
    should "show the link to add a new Player" do
      assert_select "a", :content => "Add a New Player", :attributes => {:href => new_player_path}
    end
  ...
  context "When showing the form to create a New Player" do
    ...
    should "Correctly display the Header" do
      assert_select "h2", "Creating a Player"
    end
  ...
  context "When creating a Player" do
    should "be able to create a Player" do
      assert_difference('Player.count') do
        post :create, player: {first_name: "Another", last_name: "Player"}
      end
